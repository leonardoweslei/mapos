...
class Os extends CI_Controller
{
    ...
    function gerenciar()
    {
        ...
        $this->data['results'] = $this->os_model->getOs(
            'os',
            'idOs,dataInicial,dataFinal,dataExecucao,garantia,descricaoProduto,defeito,status,observacoes,laudoTecnico',
            $where_array,
            $config['per_page'],
            $this->uri->segment(3)
        );
        ...
        if ($this->form_validation->run('os') == false) {
            ...
            try {
                ...
                if ($dataExecucao) {
                    ...
                    $dataExecucao = $dataExecucao[2] . '-' . $dataExecucao[1] . '-' . $dataExecucao[0];
                } else {
            ...
            $data = array(
                ...
                'horaExecucaoInicial' => set_value('horaExecucaoInicial'),
                'horaExecucaoFinal'   => set_value('horaExecucaoFinal'),
                'tipoAtendimento'     => set_value('tipoAtendimento'),
                'setorDepartamento'   => set_value('setorDepartamento'),
    ...
    public function adicionarAjax()
    {
        ...
        if ($this->form_validation->run('os') == false) {
            ...
            $data = array(
                ...
                'dataExecucao'        => set_value('dataExecucao'),
                'horaExecucaoInicial' => set_value('horaExecucaoInicial'),
                'horaExecucaoFinal'   => set_value('horaExecucaoFinal'),
                'tipoAtendimento'     => set_value('tipoAtendimento'),
                'setorDepartamento'   => set_value('setorDepartamento'),
    ...
    function editar()
    {
        ...
        if ($this->form_validation->run('os') == false) {
            $this->data['custom_error'] = (validation_errors() ? '<div class="form_error">' . validation_errors(
                ) . '</div>' : false);
        } else {
